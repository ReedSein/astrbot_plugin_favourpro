...
class FavourProPlugin(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        self.block_pattern = re.compile(
            r"\[\s*(?=.*(?:Favour:|Attitude:|Relationship:))[\s\S]*?\]",
            re.DOTALL
        )
        ...
        self.attitude_pattern = re.compile(r"Attitude:\s*(.+?)(?=\s*,\s*Relationship:|\])", re.DOTALL)

        # Relationship的值，就是它后面直到 "]" 之前的所有内容
        self.relationship_pattern = re.compile(r"Relationship:\s*(.+?)(?=\s*\])", re.DOTALL)

        # 兜底清理模式：匹配包含 Favour: 或 Attitude: 或 Relationship: 的任何方括号块
        self.cleanup_pattern = re.compile(
            r"\[[\s\S]*?(?:Favour:|Attitude:|Relationship:)[\s\S]*?\]",
            re.DOTALL
        )

        # 通用方括号清理模式：匹配所有方括号 [] 及其内部内容
    ...
    async def add_context_prompt(self, event: AstrMessageEvent, req: ProviderRequest):
        ...
        # 从配置中读取模板，并格式化当前状态提示词
        context_template = self.config.get(
            "context_prompt_template", 
            "[当前状态] 你与该用户的关系是：{relationship}，好感度为 {favour}，你对他的印象是：{attitude}。"
        )
        context_prompt = context_template.format(
            relationship=state.get('relationship', '未知'),
            favour=state.get('favour', 0),
            attitude=state.get('attitude', '未知')
        )

        # 从配置中读取机密指令
        instruction_prompt = self.config.get(
    ...
    async def process_llm_response(self, event: AstrMessageEvent, resp: LLMResponse):
        """
        ...
        # 如果没有找到任何看起来像状态块的东西，尝试使用兜底清理模式进行清理
        if not block_match:
            cleanup_match = self.cleanup_pattern.search(original_text)
            if cleanup_match:
                # 找到兜底匹配，直接清理文本并返回，不进行状态更新
                block_text = cleanup_match.group(0)
                resp.completion_text = original_text.replace(block_text, '').strip()
            return
        ...
        # 5. 通用清理：移除所有剩余的方括号内容
        resp.completion_text = self.universal_cleanup_pattern.sub(
            '', 
    ...
    async def on_llm_resp(self, event: AstrMessageEvent, resp: LLMResponse):
        ...
        逻辑: 查找 -> 清理 -> 解析 -> 更新
        """
        await self.process_llm_response(event, resp)

    @filter.on_message_received(priority=-1000)
    async def on_msg_received_cleanup(self, event: AstrMessageEvent):
        """
        最终、最底层的兜底清理钩子。
        如果所有 on_llm_response 和 on_decorating_result 都被绕过，
        尝试在消息被接收时进行清理。
        """
        # 仅对来自 LLM 的回复进行处理，避免误伤用户消息
        if event.is_llm_response:
            text = event.get_plain_text()
            if not text: return

            # 检查是否包含状态块
            if self.universal_cleanup_pattern.search(text):
                logger.warning("在 on_message_received 阶段检测到未清理的状态块，进行最终清理。")
                
                # 仅进行清理，不进行状态更新（因为状态更新应该在 on_llm_response 完成）
                ...
                event.chain.clear()
                event.chain.extend(new_message_chain)
    ...
    @filter.command("查询好感")
    async def query_status(self, event: AstrMessageEvent, user_id: Optional[str] = None):
        """查询自己或指定用户的状态。如果未提供 user_id，则查询自己的状态。"""
        session_id = self._get_session_id(event)
        target_user_id = str(user_id or event.get_sender_id()).strip()
        state = self.manager.get_user_state(target_user_id, session_id)
